...
import { Text, HStack, Flex, Center } from '@chakra-ui/react';
...
function PhaseTab({
	...
	const _renderModalTitle = () => {
		return <ModalHeader>
			...
			<span>Add Test Items ({project} / {phases[0].value})</span>
		</ModalHeader>;
	};
	const _renderFooter = () => {
		return <ModalFooter>
	...
	function _handleChangePage(index) {
		if (phases) {
			fetchTaskItems(phases[0].id, { pageNumber: index, pageSize: PageSize, category });
		}
	}

	function _handleChangePageSize(pageSize) {
		if (phases) {
			fetchTaskItems(phases[0].id, { pageNumber: 1, pageSize: pageSize, category });
		}
	}
	...
	useEffect(() => {
		if (addTestItemLoadingStatus === API_STATUE.COMPLETED) {
